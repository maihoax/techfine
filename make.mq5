...
void OnTick()
{
   ...
   if(openPositions == 0)
   {
      ...
      if(allowNewBatch)
         batchStartBalance = AccountBalance();
   }
   ...
   if(TimeCurrent() - lastOrderTime >= EntryInterval && openPositions < BatchSize && allowNewBatch)
   {
      ...
      {
         double risk = AccountBalance() * RiskPerTradePercent / 100.0;
         double tickValue = SymbolInfoDouble(InpSymbol, SYMBOL_TRADE_TICK_VALUE);
         if(tickValue > 0)
            lot = MathMin(LotSize, NormalizeDouble(risk/(slPoints*_Point*tickValue), 2));
      }

      if(trendingUp)
      {
         trade.Buy(lot, InpSymbol, 0,
                   NormalizeDouble(Bid - slPoints*_Point, _Digits),
                   NormalizeDouble(Bid + tpPoints*_Point, _Digits),
                   NULL, Magic);
         lastOrderTime = TimeCurrent();
      }
      else if(trendingDown)
      {
         trade.Sell(lot, InpSymbol, 0,
                    NormalizeDouble(Ask + slPoints*_Point, _Digits),
                    NormalizeDouble(Ask - tpPoints*_Point, _Digits),
                    NULL, Magic);
         lastOrderTime = TimeCurrent();
      }
   }
...
void ManageTrailingStops()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      double profitPoints = (type==POSITION_TYPE_BUY ? price - entry : entry - price) / _Point;
      ...
      if(steps > 0)
      {
         if(type==POSITION_TYPE_BUY)
            newSL = entry + steps * TrailingStepPoints * _Point;
         else
            newSL = entry - steps * TrailingStepPoints * _Point;
      }
      ...
      if(type==POSITION_TYPE_BUY)
      {
         if(newSL > sl + _Point && newSL < price)
...
bool MarketVolatile()
{
   ...
   return (change > VolatilityThreshold * _Point);
}
...
void CheckTelegramCommands()
{
   ...
   {
      string info = "Balance:" + DoubleToString(AccountBalance(), 2) +
   ...
   int p = StringFind(json, "update_id":);
   if(p >= 0)
